...
| 层 | 目录 | 职责 |
|---|---|---|
| 入口 | `main.py` | FastAPI 实例、lifespan（启动时初始化DB/发现插件/启动调度器）、CORS、路由注册、通知渠道注册、全局异常处理器 |
...
| 核心 | `core/` | security(JWT+bcrypt)、scheduler(30分钟轮询)、plugin_loader(自动发现)、docker_manager、notification_engine、custom_route、exceptions |
...
| 层 | 目录 | 职责 |
|---|---|---|
| 入口 | `App.tsx` | QueryClientProvider + BrowserRouter + ProtectedRoute(鉴权守卫，检查 localStorage token) + ToastProvider |
