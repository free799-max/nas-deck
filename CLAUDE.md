...
| 业务服务 | `services/` | 按领域拆分的业务逻辑：container_service、image_service、host_service、pull_task_service、compose_service、compose_discovery |
...
services/
├── container_service.py # 容器相关逻辑
├── image_service.py     # 镜像相关逻辑
├── host_service.py      # 宿主机信息
├── pull_task_service.py # 镜像拉取任务管理
├── compose_service.py   # Compose 项目与 CLI 操作
└── compose_discovery.py # 外部 Compose 项目自动发现
```

`app/core/docker_manager.py` 与 `app/core/compose_manager.py` 目前保留为兼容入口，内部重新导出 `services/` 中的实现，避免外部导入和测试一次性大面积改动。
